                                   ...
                                   hard.thresholds=NULL)
{
  if(use.lod){
    ...
    plot.this <- "LOD"
    this.ylab <- "LOD"
    if(print.median.band){
      MI <- scan.object$MI.LOD[,scan.object$chr == chr]
    }
  }
  else{
    outcome <- -log10(scan.object$p.value[scan.object$chr == chr])
    plot.this <- "p.value"
    this.ylab <- expression("-log"[10]*"P")
    if(print.median.band){
      MI <- -log10(scan.object$MI.LOD[,scan.object$chr == chr])
    }
  }
  
  pos <- ifelse(rep(scale=="Mb", length(outcome)), main.object$pos$Mb, main.object$pos$cM)[scan.object$chr == chr]
  ...
  if(length(non.mi.scan.list) > 1){
    for(i in 2:length(non.mi.scan.list)){
      if(use.lod){
        compare.shift <- 0
        compare.outcome <- unlist(non.mi.scan.list[[i]]["LOD"])[order.i]
  ...
  if(use.legend){
    ...
    if(!is.null(hard.thresholds)){
      for(i in 1:length(hard.thresholds)){
        abline(h=hard.thresholds[i], col=main.colors[i], lty=2)
      }
    }
  }
}
